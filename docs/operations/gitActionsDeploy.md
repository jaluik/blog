...
我们这里就是利用推送到主分支的这个事件，来启用 pnpm，并触发`pnpm install --frozen-lockfile && pnpm build`自动安装依赖并打包，最终把构建的 build 产物推送到腾讯云 COS 存储空间上。
...
2. GitHub Actions 就会执行`pnpm install --frozen-lockfile && pnpm build`生成临时`build`文件夹
